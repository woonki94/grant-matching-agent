...
from graph_rag.faculty.sync_neo4j import _clean_text as _clean_keyword_text
from graph_rag.faculty.sync_neo4j import _embed_text_values
...
def _publication_id_from_snippet_id(snippet_id: str) -> Optional[int]:
    sid = str(snippet_id or "").strip()
    if not sid.startswith("pub|"):
        return None
    parts = sid.split("|")
    if len(parts) < 4:
        return None
    ...
    if isinstance(domains_v2, list) or isinstance(specs_v2, list):
        ...
        for item in list(specs_v2 or []):
            if isinstance(item, dict):
                ...
                _upsert_spec(section="research", value=item, weight=None, snippet_ids={}, domains={})
    ...
    for section in ("research", "application"):
        ...
        for item in list(sec.get("domain") or []):
            if isinstance(item, dict):
                ...
                _upsert_domain(section=section, value=item, weight=None, snippet_ids={})
        for item in list(sec.get("specialization") or []):
            if isinstance(item, dict):
                ...
                _upsert_spec(section=section, value=item, weight=None, snippet_ids={}, domains={})

    # Ensure any specialization-linked domain exists as a domain node.
...
def _load_faculties(
    *,
    ...
    with SessionLocal() as sess:
        ...
        if int(faculty_id or 0) > 0:
            q = q.filter(Faculty.faculty_id == int(faculty_id))
        elif bool(sync_all):
            ...
            raise ValueError("Set --faculty-id or --all.")
    ...
    for fac in rows:
        try:
            fid = int(getattr(fac, "faculty_id", 0) or 0)
        except Exception:
            continue
        email = str(getattr(fac, "email", "") or "").strip().lower()
        if fid <= 0 or not email:
            continue
        payload = (getattr(fac, "keyword", None) and getattr(fac.keyword, "keywords", None)) or {}
        if not isinstance(payload, dict):
            payload = {}
        out.append(
            {
...
def sync_faculty_keyword_links_to_neo4j(
    *,
    driver,
    database: str,
    ...
    email = str(faculty_row["email"]).strip().lower()
    keywords = dict(faculty_row.get("keywords") or {})
    domain_rows, spec_rows = _parse_keywords_payload(keywords)

    # Refresh all uploader-managed edges for this faculty scope.
    driver.execute_query(
        """
        MATCH ()-[r]->()
        WHERE r.scope_faculty_id = $faculty_id
          AND type(r) IN $types
        DELETE r
        """,
        parameters_={"faculty_id": faculty_id, "types": CUSTOM_EDGE_TYPES},
        database_=database,
    )
    driver.execute_query(
        """
    ...
    if domain_terms:
        ...
        driver.execute_query(
            ...
            MERGE (g:FacultyDomainGate {faculty_id: $faculty_id})
            SET
                g.email = $email,
                g.section = 'general',
                g.domain_terms = $domain_terms,
                g.domain_text = $domain_text,
                g.domain_count = size($domain_terms),
                g.embedding = $embedding,
                g.embedding_model = $embedding_model,
                g.updated_at = datetime()
            MERGE (f)-[r:HAS_DOMAIN_GATE]->(g)
        ...
        driver.execute_query(
            """
            MATCH (f:Faculty {faculty_id: $faculty_id})-[r:HAS_DOMAIN_GATE]->(g:FacultyDomainGate {faculty_id: $faculty_id})
            DELETE r
            WITH g
            DETACH DELETE g
            """,
    ...
    # Upsert keyword nodes and faculty->keyword relations with weights.
    for (section, bucket), relation in KEYWORD_RELATIONS.items():
        ...
        driver.execute_query(
            ...
            """,
            parameters_={"faculty_id": faculty_id, "rows": rows},
            database_=database,
        )
    ...
    domain_pub_rows: List[Dict[str, Any]] = []
    ...
    for row in domain_rows:
        ...
        for sid, score in dict(row.get("snippet_ids") or {}).items():
            ...
            if not sid_s:
                continue
            pub_id = _publication_id_from_snippet_id(sid_s)
            if pub_id is None:
                ...
                key = (section, domain_value, pub_id)
                if key in seen_domain_pub:
                    continue
                seen_domain_pub.add(key)
                domain_pub_rows.append(
                    {
                        "domain_value": domain_value,
                        "domain_section": section,
    ...
    pub_spec_rows: List[Dict[str, Any]] = []
    seen_chunk_spec = set()
    seen_pub_spec = set()
    ...
    for row in spec_rows:
        ...
        for sid, score in dict(row.get("snippet_ids") or {}).items():
            ...
            if not sid_s:
                continue
            evidence_score = _coerce_weight(score, default=0.8)
            pub_id = _publication_id_from_snippet_id(sid_s)
            if pub_id is None:
                ...
                key = (spec_section, spec_value, pub_id)
                if key in seen_pub_spec:
                    continue
                seen_pub_spec.add(key)
                pub_spec_rows.append(
                    {
                        "specialization_section": spec_section,
                        "specialization_value": spec_value,
                        "publication_id": int(pub_id),
    ...
    if domain_chunk_rows:
        records, _, _ = driver.execute_query(
            """
            UNWIND $rows AS row
            MATCH (d:FacultyKeyword {
                value: row.domain_value,
                section: row.domain_section,
                bucket: 'domain'
            })
            MATCH (c:FacultyTextChunk {chunk_id: row.chunk_id})
            WHERE toLower(coalesce(c.email, '')) = $email
            MERGE (d)-[r:DOMAIN_SUPPORTED_BY_FACULTY_CHUNK {
            ...
            """,
            parameters_={"faculty_id": faculty_id, "email": email, "rows": domain_chunk_rows},
            database_=database,
        )
    ...
    if domain_pub_rows:
        records, _, _ = driver.execute_query(
            ...
            MERGE (d)-[r:DOMAIN_SUPPORTED_BY_FACULTY_PUBLICATION {
                scope_faculty_id: $faculty_id,
                domain_value: row.domain_value,
                domain_section: row.domain_section,
            ...
            RETURN count(r) AS linked_count
            """,
            parameters_={"faculty_id": faculty_id, "rows": domain_pub_rows},
            database_=database,
        )
        domain_pub_linked = _link_count(records)
    ...
    if domain_spec_rows:
        records, _, _ = driver.execute_query(
            ...
            MATCH (s:FacultyKeyword {
                value: row.specialization_value,
                section: row.specialization_section,
                bucket: 'specialization'
            })
            MERGE (d)-[r:FACULTY_DOMAIN_HAS_SPECIALIZATION {
                scope_faculty_id: $faculty_id,
                domain_value: row.domain_value,
                domain_section: row.domain_section,
            ...
            RETURN count(r) AS linked_count
            """,
            parameters_={"faculty_id": faculty_id, "rows": domain_spec_rows},
            database_=database,
        )
    ...
    if chunk_spec_rows:
        records, _, _ = driver.execute_query(
            ...
            MERGE (c)-[r:FACULTY_CHUNK_SUPPORTS_SPECIALIZATION {
                scope_faculty_id: $faculty_id,
                chunk_id: row.chunk_id,
                specialization_value: row.specialization_value,
                specialization_section: row.specialization_section
            ...
            RETURN count(r) AS linked_count
            """,
            parameters_={"faculty_id": faculty_id, "email": email, "rows": chunk_spec_rows},
            database_=database,
        )
    ...
    if pub_spec_rows:
        records, _, _ = driver.execute_query(
            ...
            MERGE (p)-[r:FACULTY_PUBLICATION_SUPPORTS_SPECIALIZATION {
                scope_faculty_id: $faculty_id,
                publication_id: row.publication_id,
                specialization_value: row.specialization_value,
                specialization_section: row.specialization_section
            ...
            RETURN count(r) AS linked_count
            """,
            parameters_={"faculty_id": faculty_id, "rows": pub_spec_rows},
            database_=database,
        )
        pub_spec_linked = _link_count(records)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sync faculty keyword linkage edges from relational keywords JSON into Neo4j. "
            "Creates: domain->chunk, domain->specialization, chunk->specialization."
        )
    )
    parser.add_argument("--faculty-id", type=int, default=0, help="Sync one faculty by faculty_id.")
...
def main() -> int:
    ...
    with GraphDatabase.driver(
        ...
        for fac in faculties:
            try:
                synced.append(
                    sync_faculty_keyword_links_to_neo4j(
                        driver=driver,
                ...
                errors.append(
                    {
                        "faculty_id": int(fac.get("faculty_id") or 0),
                        "email": str(fac.get("email") or ""),
    ...
    totals = {
        ...
        "domain_publication_candidates": sum(int(x.get("counts", {}).get("domain_publication_candidates", 0)) for x in synced),
        ...
        "publication_specialization_candidates": sum(
            int(x.get("counts", {}).get("publication_specialization_candidates", 0)) for x in synced
        ),
    ...
    if not bool(args.json_only):
        print("Faculty keyword link sync complete.")
