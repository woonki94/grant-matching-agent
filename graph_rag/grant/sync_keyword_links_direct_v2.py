...
from graph_rag.grant.sync_keyword_links_v2 import _load_opportunities, sync_grant_keyword_links_to_neo4j
...
class GrantKeywordLinkNeo4jSink(GrantKeywordGeneratorV2):
    def __init__(
        self,
        *,
        driver,
    ...
    def save_grant_keywords(
        ...
        *,
        ...
        result = sync_grant_keyword_links_to_neo4j(
            driver=self._driver,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run GrantKeywordGeneratorV2 and sync keyword linkage edges directly to Neo4j "
    ...
    parser.add_argument("--all", action="store_true", help="Run all grant rows.")
...
def main() -> int:
    ...
    target_ids = [str(x["opportunity_id"]).strip() for x in opp_rows if str(x.get("opportunity_id") or "").strip()]
    ...
    with GraphDatabase.driver(
        ...
        generator = GrantKeywordLinkNeo4jSink(
            driver=driver,
        ...
        if len(target_ids) == 1:
            oid = str(target_ids[0])
            generator.run_grant_keyword_pipeline(
                opportunity_id=oid,
                max_context_chars=max_context_chars,
                persist=True,
            ...
            run_results = [{"opportunity_id": oid, "ok": True}]
        else:
            def _run_one(oid: str) -> Dict[str, Any]:
                generator.run_grant_keyword_pipeline(
                ...
                return {"opportunity_id": str(oid), "ok": True}

            def _on_error(_idx: int, oid: str, e: Exception) -> Dict[str, Any]:
                logger.exception("[sync_keyword_links_direct_v2.grant] opportunity_id=%s failed", oid)
                return {
                    "opportunity_id": str(oid),
                    "ok": False,
            ...
            failed_ids = [str(x["opportunity_id"]) for x in run_results if not bool(x.get("ok"))]
            summary = {
    ...
    if not bool(args.json_only):
        print("Grant direct keyword->link sync complete.")
