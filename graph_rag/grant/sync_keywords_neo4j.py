...
def sync_grant_keywords_to_neo4j(
    *,
    driver,
    settings_neo4j,
    opp,
    ...
    opportunity_id = str(getattr(opp, "opportunity_id", "") or "").strip()
    if not opportunity_id:
        raise ValueError("Opportunity must have opportunity_id.")
    ...
    if support_rows:
        records, _, _ = driver.execute_query(
            ...
            """,
            parameters_={"opportunity_id": opportunity_id, "rows": support_rows},
            database_=settings_neo4j.database,
        )
        if records:
            try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sync grant keywords + snippet-based keyword->chunk links into Neo4j.")
    parser.add_argument("--opportunity-id", type=str, default="", help="Sync one opportunity id.")
    parser.add_argument("--all", action="store_true", help="Sync all opportunity rows.")
    ...
    parser.add_argument("--stop-on-error", action="store_true", help="Stop on first grant sync error.")
    parser.add_argument("--embedding-batch-size", type=int, default=12, help="Embedding batch size for keyword nodes.")
    parser.add_argument("--skip-embeddings", action="store_true", help="Skip keyword embeddings on GrantKeyword nodes.")
...
def main() -> int:
    ...
    if not rows:
        raise RuntimeError("No grant rows found for requested sync scope.")
    ...
    if not args.json_only:
        print("Grant keyword sync complete.")
