...
def retrieve_grants_ranked_by_spec_edges(
    *,
    ...
    query = """
        ...
        WITH
            ...
            max(coalesce(m.score, 0.0)) AS best_edge_score,
            count(*) AS edge_count,
            collect({
        ...
        RETURN
            ...
            edge_count,
            matched_pairs
        ORDER BY rank_score DESC, best_edge_score DESC, edge_count DESC
        LIMIT $limit
    """
        ...
        rows.append(
            {
                ...
                "best_edge_score": float(item.get("best_edge_score") or 0.0),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Retrieve grants ranked by FACULTY_SPEC_MATCHES_GRANT_SPEC edge scores "
            "for one faculty (ranked by sum of edge scores)."
        )
    )
...
def main() -> int:
    ...
    payload = retrieve_grants_ranked_by_spec_edges(
    ...
    if not args.json_only:
        print("Grant retrieval from spec-match edges complete.")
