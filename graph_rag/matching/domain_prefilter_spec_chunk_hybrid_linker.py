...
class _CrossEncoderScorer:
    def __init__(self, *, model_name: str, batch_size: int):
        try:
            ...
            raise RuntimeError(
                "Hybrid scorer requires sentence-transformers. "
...
def _fetch_faculty_evidence_rows(
    *,
    ...
    records, _, _ = driver.execute_query(
        ...
        RETURN DISTINCT evidence_kind, evidence_id, source_type, embedding, text
...
def _prepare_grant_chunks(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in rows or []:
        ...
        if not chunk_id or vec.size == 0 or not text:
            continue
        out.append(
            {
                "chunk_id": chunk_id,
...
def _prepare_faculty_evidence(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in rows or []:
        ...
        if evidence_kind not in {"chunk", "publication"}:
            continue
        if not evidence_id or vec.size == 0 or not text:
            continue
        out.append(
            {
...
def _rank_faculty_spec_to_grant_chunks(
    *,
    ...
    safe_min_cosine = max(0.0, min(1.0, float(min_cosine_sim)))
    safe_min_attention = max(0.0, min(1.0, float(min_attention_score)))
    ...
    prepared_chunks = [
        row
        for row in grant_chunks
        if _coerce_vector(row.get("vector")).size > 0 and _clean_text(row.get("text"))
    ]
    ...
    for fk in faculty_specs:
        ...
        for idx in order:
            ...
            left_text = _truncate_text(fk.get("keyword_value"), safe_max_pair_chars)
            right_text = _truncate_text(chunk_row.get("text"), safe_max_pair_chars)
            ...
            pair_inputs.append((left_text, right_text))
            if len(candidate_rows) >= safe_top_k:
                break
        ...
        for idx, row in enumerate(candidate_rows):
            attn = _safe_unit_float(attention_scores[idx], default=0.0)
            if attn < safe_min_attention:
                continue
            row["attention_score"] = attn
            links.append(row)
    ...
    *,
    ...
    for gk in grant_specs:
        ...
        for idx in order:
            ...
            left_text = _truncate_text(gk.get("keyword_value"), safe_max_pair_chars)
            right_text = _truncate_text(ev_row.get("text"), safe_max_pair_chars)
            if not left_text or not right_text:
                continue
    ...
    *,
    ...
    records, _, _ = driver.execute_query(
        ...
        SET
            ...
            r.method = 'domain_prefilter_plus_hybrid_spec_to_grant_chunk',
        ...
        parameters_={
            ...
            "opportunity_id": _clean_text(opportunity_id),
            "rows": rows,
            "min_cosine_sim": max(0.0, min(1.0, float(min_cosine_sim))),
            "min_attention_score": max(0.0, min(1.0, float(min_attention_score))),
    ...
    if chunk_rows:
        records, _, _ = driver.execute_query(
            ...
            SET
                ...
                r.min_attention_score = $min_attention_score,
                r.min_domain_sim = $min_domain_sim,
                r.method = 'domain_prefilter_plus_hybrid_grant_spec_to_fac_evidence',
                r.updated_at = datetime()
            RETURN count(r) AS linked_count
            """,
            parameters_={
...
def run_domain_prefilter_spec_chunk_hybrid_linker(
    *,
    ...
    safe_min_cosine_sim = max(0.0, min(1.0, float(min_cosine_sim)))
    safe_min_attention_score = max(0.0, min(1.0, float(min_attention_score)))
    ...
    safe_cross_encoder_model = _clean_text(cross_encoder_model) or "BAAI/bge-reranker-v2-m3"
    safe_cross_encoder_batch = _safe_limit(cross_encoder_batch_size, default=16, minimum=1, maximum=512)
    safe_max_pair_text_chars = _safe_limit(max_pair_text_chars, default=1800, minimum=200, maximum=12000)
    ...
    with GraphDatabase.driver(
        ...
        scorer = _CrossEncoderScorer(
            model_name=safe_cross_encoder_model,
        ...
        for faculty_email in targets_faculty:
            ...
            for candidate in candidates:
                ...
                fac_to_grant_rows = _rank_faculty_spec_to_grant_chunks(
                    ...
                    scorer=scorer,
                    min_cosine_sim=safe_min_cosine_sim,
                    min_attention_score=safe_min_attention_score,
                    top_k_per_keyword=safe_top_k_f2g,
                    domain_gate_score=float(candidate.get("domain_gate_score") or 0.0),
                    max_pair_text_chars=safe_max_pair_text_chars,
                )
    ...
    return {
        "params": {
            ...
            "min_keyword_confidence": safe_min_keyword_confidence,
            "cross_encoder_model": safe_cross_encoder_model,
            "cross_encoder_batch_size": safe_cross_encoder_batch,
            "max_pair_text_chars": safe_max_pair_text_chars,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Two-stage matcher: domain prefilter first, then write hybrid cross-chunk specialization edges "
            "storing both cosine_sim and attention_score (no explicit final score)."
        )
    )
    ...
    parser.add_argument("--min-domain-sim", type=float, default=0.2, help="Domain prefilter threshold.")
    ...
    parser.add_argument("--min-cosine-sim", type=float, default=0.3, help="Minimum cosine similarity for stage-2 candidate links.")
    parser.add_argument("--min-attention-score", type=float, default=0.3, help="Minimum cross-encoder attention score for final kept links.")
    parser.add_argument("--top-k-fac-to-grant-chunk", type=int, default=20, help="Top grant chunks kept per faculty specialization keyword.")
    parser.add_argument("--top-k-grant-to-fac-evidence", type=int, default=20, help="Top faculty evidence items kept per grant specialization keyword.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Cross-encoder batch size for pair scoring.",
    )
    parser.add_argument(
        "--max-pair-text-chars",
        type=int,
        default=1800,
        help="Max chars kept for each text side before attention scoring.",
    )
...
def main() -> int:
    ...
    payload = run_domain_prefilter_spec_chunk_hybrid_linker(
    ...
    if not args.json_only:
        ...
        print("Domain-prefilter hybrid chunk linker complete.")
