...
class _CrossEncoderScorer:
    def __init__(self, *, model_name: str, batch_size: int):
        try:
            ...
            raise RuntimeError(
                "Cross-encoder scoring requires sentence-transformers. "
...
def _fetch_faculty_evidence_rows(
    *,
    ...
    records, _, _ = driver.execute_query(
        ...
        RETURN DISTINCT evidence_kind, evidence_id, source_type, text
...
def _rank_faculty_spec_to_grant_chunks(
    *,
    ...
    safe_min_attn = max(0.0, min(1.0, float(min_attention_score)))
    ...
    safe_pair_chars = _safe_limit(max_pair_text_chars, default=1800, minimum=200, maximum=12000)

    if not faculty_specs or not grant_chunks:
        return links

    prepared_chunks = [row for row in grant_chunks if _clean_text(row.get("text"))]
    if not prepared_chunks:
        return links

    for fk in faculty_specs:
        fk_kw = _clean_text(fk.get("keyword_value"))
        if not fk_kw:
            continue

        pairs: List[Tuple[str, str]] = []
        candidates: List[Dict[str, Any]] = []
        left_text = _truncate_text(fk_kw, safe_pair_chars)
        for gc in prepared_chunks:
            right_text = _truncate_text(_clean_text(gc.get("text")), safe_pair_chars)
        ...
        kept = 0
        for idx in order:
            attention_score = float(scores[int(idx)])
            if attention_score < safe_min_attn:
                continue

            chunk_row = candidates[int(idx)]
            ...
            score = max(0.0, min(1.0, attention_score * fk_weight * fk_conf * safe_domain))
...
def _rank_grant_spec_to_faculty_evidence(
    *,
    ...
    prepared_evidence = [row for row in faculty_evidence if _clean_text(row.get("text"))]
    if not prepared_evidence:
        return links

    for gk in grant_specs:
        gk_kw = _clean_text(gk.get("keyword_value"))
        if not gk_kw:
            continue

        pairs: List[Tuple[str, str]] = []
        candidates: List[Dict[str, Any]] = []
        left_text = _truncate_text(gk_kw, safe_pair_chars)
        for ev in prepared_evidence:
            right_text = _truncate_text(_clean_text(ev.get("text")), safe_pair_chars)
            if not left_text or not right_text:
                continue
            pairs.append((left_text, right_text))
        ...
        for idx in order:
            ...
            score = max(0.0, min(1.0, attention_score * gk_weight * gk_conf * safe_domain))
    ...
    *,
    ...
    records, _, _ = driver.execute_query(
        ...
        SET
            ...
            r.method = 'domain_prefilter_plus_attention_spec_to_grant_chunk',
    ...
    if chunk_rows:
        records, _, _ = driver.execute_query(
            ...
            SET
                ...
                r.min_attention_score = $min_attention_score,
                r.min_domain_sim = $min_domain_sim,
                r.method = 'domain_prefilter_plus_attention_grant_spec_to_fac_evidence',
                r.updated_at = datetime()
            RETURN count(r) AS linked_count
            """,
            parameters_={
...
def run_domain_prefilter_spec_chunk_attention_linker(
    *,
    ...
    safe_pair_text_chars = _safe_limit(max_pair_text_chars, default=1800, minimum=200, maximum=12000)
        ...
        for faculty_email in targets_faculty:
            ...
            for candidate in candidates:
                ...
                fac_to_grant_rows = _rank_faculty_spec_to_grant_chunks(
                    ...
                    max_pair_text_chars=safe_pair_text_chars,
                )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "scored by cross-encoder attention (faculty keyword -> grant chunks, "
            "grant keyword -> faculty chunk/publication)."
        )
    )
    ...
    parser.add_argument("--min-attention-score", type=float, default=0.55, help="Minimum cross-encoder attention score for specialization->opposite-evidence links.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1800,
        help="Max characters kept for each side of a pair before cross-encoder scoring.",
    )
...
def main() -> int:
    ...
    payload = run_domain_prefilter_spec_chunk_attention_linker(
    ...
    if not args.json_only:
        ...
        print("Domain-prefilter attention chunk linker complete.")
