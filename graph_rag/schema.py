...
from graph_rag.common import Neo4jSettings, load_dotenv_if_present, read_neo4j_settings
...
GRANT_SCHEMA_STATEMENTS: List[Tuple[str, str]] = [
    ...
    (
        "grant_text_chunk_id_unique",
        "CREATE CONSTRAINT grant_text_chunk_id_unique IF NOT EXISTS "
        "FOR (c:GrantTextChunk) REQUIRE c.chunk_id IS UNIQUE",
    ),
    (
    ...
    (
        "grant_keyword_bucket_index",
        "CREATE INDEX grant_keyword_bucket IF NOT EXISTS "
        "FOR (k:GrantKeyword) ON (k.bucket)",
    ),
    (
        "grant_chunk_source_type_index",
        "CREATE INDEX grant_chunk_source_type IF NOT EXISTS "
        "FOR (c:GrantTextChunk) ON (c.source_type)",
    ),
]
...
FACULTY_SCHEMA_STATEMENTS: List[Tuple[str, str]] = [
    ...
    (
        "faculty_text_chunk_id_unique",
        "CREATE CONSTRAINT faculty_text_chunk_id_unique IF NOT EXISTS "
        "FOR (c:FacultyTextChunk) REQUIRE c.chunk_id IS UNIQUE",
    ),
    (
    ...
    (
        "faculty_keyword_bucket_index",
        "CREATE INDEX faculty_keyword_bucket IF NOT EXISTS "
        "FOR (k:FacultyKeyword) ON (k.bucket)",
    ),
    (
        "faculty_chunk_source_type_index",
        "CREATE INDEX faculty_chunk_source_type IF NOT EXISTS "
        "FOR (c:FacultyTextChunk) ON (c.source_type)",
    ),
]
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Initialize shared Neo4j schema for GraphRAG.")
...
def main() -> int:
    ...
    print("GraphRAG Neo4j schema ready.")
    return 0
