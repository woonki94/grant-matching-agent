...
MODEL_ID="${MODEL_ID:-/path/to/stage1_checkpoint}"
...
# Loss calibration knobs (same defaults as train.sh)
LOSS_KL_WEIGHT="${LOSS_KL_WEIGHT:-0.7}"
LOSS_PAIR_WEIGHT="${LOSS_PAIR_WEIGHT:-0.20}"
LOSS_MSE_WEIGHT="${LOSS_MSE_WEIGHT:-0.45}"
LISTWISE_SCORE_MODE="${LISTWISE_SCORE_MODE:-raw}"        # raw | normalized
LOSS_CLUSTER_MARGIN_WEIGHT="${LOSS_CLUSTER_MARGIN_WEIGHT:-1.2}"
LOSS_CALIBRATION_BAND_WEIGHT="${LOSS_CALIBRATION_BAND_WEIGHT:-0.70}"
CLUSTER_MARGIN_HM="${CLUSTER_MARGIN_HM:-0.24}"
CLUSTER_MARGIN_ML="${CLUSTER_MARGIN_ML:-0.26}"
CLUSTER_MARGIN_HL="${CLUSTER_MARGIN_HL:-0.55}"
CALIB_BAND_MODE="${CALIB_BAND_MODE:-data_driven}"        # fixed | data_driven
...
CALIB_LOW_CEIL="${CALIB_LOW_CEIL:-0.25}"
