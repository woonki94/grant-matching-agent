...
DISTILL_INPUT_FALLBACKS = [
    "cross_encoder/dataset/splits/llm_distill_raw_test.jsonl",
    "cross_encoder/dataset/distill/llm_distill2_listwise.jsonl",
    "cross_encoder/spec_to_spec/dataset/splits/llm_distill_raw_test.jsonl",
    "cross_encoder/spec_to_spec/dataset/llm_distill_raw_scores.jsonl",
    "cross_encoder/spec_to_spec/dataset/llm_distill_listwise.jsonl",
]
DISTILL_TEST_INPUT_DEFAULT = "cross_encoder/dataset/splits/llm_distill_raw_test.jsonl"
...
def _resolve_default_finetuned_model_ref() -> str:
    ...
    # Direct/default paths first.
    for root in (base, legacy):
        candidates.append(root / "best")
        candidates.append(root)
        for i in range(1, 16):
            candidates.append(root / f"stage2_epoch_{i}")
            candidates.append(root / f"stage1_epoch_{i}")

    # Handle training outputs with append-args-to-output-dir=true:
    # cross_encoder/models/bge_reranker_distill__.../best
    for root in (base, legacy):
        parent = root.parent
        ...
        if parent.is_dir():
            for p in parent.iterdir():
                if p.is_dir() and p.name.startswith(prefix):
                    ...
                    candidates.append(p)
                    for i in range(1, 16):
                        candidates.append(p / f"stage2_epoch_{i}")
                        candidates.append(p / f"stage1_epoch_{i}")
    ...
    # Last-resort legacy path behavior.
    return _resolve_model_ref(FINETUNED_MODEL_DEFAULT)


def _resolve_distill_input_path(preferred_paths: Sequence[str]) -> Path:
    checked: List[Path] = []
    for raw in preferred_paths:
        ...
        if p.exists():
            return p
    rendered = ", ".join(str(p) for p in checked) if checked else "(none)"
...
def _iter_distill_pairs_from_obj(obj: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    ranked_docs = list(obj.get("ranked_docs") or [])
    if ranked_docs:
            ...
            out.append(
                {
                    ...
                    "source_band": _clean_text(d.get("band") or d.get("target_cluster")) or "unknown",
                }
            )
        return out
...
def _run_distill_eval(args: argparse.Namespace) -> int:
    ...
    distill_input = _resolve_distill_input_path(
        [
            _clean_text(args.distill_test_input),
            _clean_text(args.distill_input),
            DISTILL_INPUT_DEFAULT,
            *DISTILL_INPUT_FALLBACKS,
        ]
    )
