...
OUTPUT_DEFAULT = "cross_encoder/spec_to_spec/dataset/spec_facspec_sts_cache.jsonl"
MANIFEST_DEFAULT = "cross_encoder/spec_to_spec/dataset/spec_facspec_sts_cache.manifest.json"
...
def _build_cache_sts(
    *,
    ...
    model_ref = _clean_text(sts_model_id) or STS_MODEL_ID_DEFAULT
    device = _pick_device()
    ...
    with output_path.open("w", encoding="utf-8") as out_f:
        for spec in specs:
            ...
            if not query_text:
                continue
            scores = _score_docs_with_sts_ce(
                model=model,
                tokenizer=tokenizer,
                ...
                candidates.append(
                    {
                        ...
                        "fac_spec_idx": int(fac_spec["fac_spec_idx"]),
                        "section": _clean_text(fac_spec["section"]),
                        "sts_score": float(scores[int(idx)]),
                    }
                )
    ...
    manifest = {
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "source": "sts_cross_encoder",
        "prefilter_method": "sts",
        "grant_db_path": str(grant_db_path),
        "fac_db_path": str(fac_db_path),
        "output_path": str(output_path),
        "sts_model_id": model_ref,
        "top_k_per_spec": int(safe_top_k),
        "written_spec_count": int(spec_count_written),
        "candidate_count_per_spec": int(keep_k),
        "total_written_candidates": int(spec_count_written * keep_k),
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            ...
            "Default method is STS cross-encoder reranker scoring."
        )
    )
    ...
    p.add_argument(
        ...
        help="Prefilter method: sts (default) or cosine.",
    )
    p.add_argument(
        "--sts-model-id",
        type=str,
        default=STS_MODEL_ID_DEFAULT,
        help="STS cross-encoder model id/path when prefilter-method=sts.",
    )
    p.add_argument("--sts-batch-size", type=int, default=64, help="STS CE scoring batch size.")
    p.add_argument("--sts-max-length", type=int, default=512, help="STS CE tokenizer max length.")
    p.add_argument("--bge-model-id", type=str, default="", help=argparse.SUPPRESS)
    p.add_argument("--bge-batch-size", type=int, default=0, help=argparse.SUPPRESS)
    p.add_argument("--bge-max-length", type=int, default=0, help=argparse.SUPPRESS)
    p.add_argument(
        "--model-id",
        type=str,
        default="",
...
def main() -> int:
    ...
    sts_model_id = _clean_text(args.sts_model_id) or _clean_text(args.bge_model_id) or STS_MODEL_ID_DEFAULT
    sts_batch_size = _safe_int(
        args.sts_batch_size if int(args.sts_batch_size or 0) > 0 else args.bge_batch_size,
        default=64,
        minimum=1,
        maximum=4096,
    )
    sts_max_length = _safe_int(
        args.sts_max_length if int(args.sts_max_length or 0) > 0 else args.bge_max_length,
        default=512,
        minimum=32,
        maximum=4096,
    )
