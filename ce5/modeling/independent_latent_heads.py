"""Original independent-latent-head ModernCE architecture.
...
class LatentHeadConfig:
    """Inference architecture saved with a CE5 checkpoint."""
...
class LatentHeadOutput:
    """Inference outputs and raw diagnostics used by external trainers."""
...
class ModernCELatentHeadModel(nn.Module):
    ...
    def from_checkpoint(
        ...
        **pretrained_kwargs: Any,
        ...
        if version not in SUPPORTED_CHECKPOINT_FORMAT_VERSIONS:
            raise RuntimeError(
                f"Unsupported CE5 checkpoint version {version}; "
                f"supported versions are {SUPPORTED_CHECKPOINT_FORMAT_VERSIONS}"
            )
        ...
        architecture_values = dict(raw_config)
        if version == 1:
            for field_name in V1_TRAINING_CONFIG_FIELDS:
                architecture_values.pop(field_name, None)
        config = LatentHeadConfig(**architecture_values)
