...
LABELS = {
    ...
    "additional_info": ["Additional Links", "Links"],
}
...
def extract_additional_links(soup: BeautifulSoup) -> List[str]:
    links = []

    field = soup.select_one(".field--name-field-c-engr-external-links .field__items")
    if not field:
        return links

    for a in field.select("a[href]"):
        name = text_clean(a.get_text(" "))
        ...
        if not href.startswith(("http://", "https://")):
            href = urljoin(settings.osu_eng_base_url, href)
...
def parse_profile(url: str) -> Dict:
    ...
    return {
        ...
        "biography": biography,
        "awards": awards,
        "additional_info": additional_links,
        "source_url": url,
    }
