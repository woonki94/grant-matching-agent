...
# ==========================================================
# CE Train2 runner (domain + method, pairwise + listwise)
# ==========================================================
# Usage:
#   bash ce/train2.sh
#
# Optional overrides:
#   STAGE1_EPOCHS=4 STAGE2_EPOCHS=6 LOSS_PAIR_WEIGHT=0.25 \
#   LOSS_MSE_WEIGHT=0.20 WANDB_PROJECT=ce_distill \
#   bash ce/train2.sh
...
RAW_TRAIN_INPUT="${RAW_TRAIN_INPUT:-ce/dataset/splits/llm_distill_domain_listwise_train.jsonl}"
RAW_VAL_INPUT="${RAW_VAL_INPUT:-ce/dataset/splits/llm_distill_domain_listwise_val.jsonl}"
RAW_TEST_INPUT="${RAW_TEST_INPUT:-ce/dataset/splits/llm_distill_domain_listwise_test.jsonl}"
METHOD_RAW_TRAIN_INPUT="${METHOD_RAW_TRAIN_INPUT:-ce/dataset/splits/llm_distill_method_listwise_train.jsonl}"
METHOD_RAW_VAL_INPUT="${METHOD_RAW_VAL_INPUT:-ce/dataset/splits/llm_distill_method_listwise_val.jsonl}"
METHOD_RAW_TEST_INPUT="${METHOD_RAW_TEST_INPUT:-ce/dataset/splits/llm_distill_method_listwise_test.jsonl}"
CONSTRAINT_RAW_TRAIN_INPUT="${CONSTRAINT_RAW_TRAIN_INPUT:-ce/dataset/splits/llm_distill_constraint_listwise_train.jsonl}"
CONSTRAINT_RAW_VAL_INPUT="${CONSTRAINT_RAW_VAL_INPUT:-ce/dataset/splits/llm_distill_constraint_listwise_val.jsonl}"
CONSTRAINT_RAW_TEST_INPUT="${CONSTRAINT_RAW_TEST_INPUT:-ce/dataset/splits/llm_distill_constraint_listwise_test.jsonl}"

PAIRWISE_TRAIN_INPUT="${PAIRWISE_TRAIN_INPUT:-ce/dataset/splits/llm_distill_domain_pairwise_train.jsonl}"
PAIRWISE_VAL_INPUT="${PAIRWISE_VAL_INPUT:-ce/dataset/splits/llm_distill_domain_pairwise_val.jsonl}"
PAIRWISE_TEST_INPUT="${PAIRWISE_TEST_INPUT:-ce/dataset/splits/llm_distill_domain_pairwise_test.jsonl}"
METHOD_PAIRWISE_TRAIN_INPUT="${METHOD_PAIRWISE_TRAIN_INPUT:-ce/dataset/splits/llm_distill_method_pairwise_train.jsonl}"
METHOD_PAIRWISE_VAL_INPUT="${METHOD_PAIRWISE_VAL_INPUT:-ce/dataset/splits/llm_distill_method_pairwise_val.jsonl}"
METHOD_PAIRWISE_TEST_INPUT="${METHOD_PAIRWISE_TEST_INPUT:-ce/dataset/splits/llm_distill_method_pairwise_test.jsonl}"
CONSTRAINT_PAIRWISE_TRAIN_INPUT="${CONSTRAINT_PAIRWISE_TRAIN_INPUT:-ce/dataset/splits/llm_distill_constraint_pairwise_train.jsonl}"
CONSTRAINT_PAIRWISE_VAL_INPUT="${CONSTRAINT_PAIRWISE_VAL_INPUT:-ce/dataset/splits/llm_distill_constraint_pairwise_val.jsonl}"
CONSTRAINT_PAIRWISE_TEST_INPUT="${CONSTRAINT_PAIRWISE_TEST_INPUT:-ce/dataset/splits/llm_distill_constraint_pairwise_test.jsonl}"
...
STAGE1_LEARNING_RATE="${STAGE1_LEARNING_RATE:-1.1e-6}"
STAGE2_LEARNING_RATE="${STAGE2_LEARNING_RATE:-4.0e-7}"
...
# Losses / stage2 behavior
LISTWISE_SCORE_MODE="${LISTWISE_SCORE_MODE:-raw}"  # raw | normalized
LOSS_KL_WEIGHT="${LOSS_KL_WEIGHT:-0.62}"
LOSS_PAIR_WEIGHT="${LOSS_PAIR_WEIGHT:-0.18}"
LOSS_MSE_WEIGHT="${LOSS_MSE_WEIGHT:-0.24}"
LOSS_CLUSTER_MARGIN_WEIGHT="${LOSS_CLUSTER_MARGIN_WEIGHT:-0.75}"
LOSS_CALIBRATION_BAND_WEIGHT="${LOSS_CALIBRATION_BAND_WEIGHT:-0.55}"
...
METHOD_LIST_LOSS_SCALE="${METHOD_LIST_LOSS_SCALE:-1.35}"
...
CALIB_MID_CENTER="${CALIB_MID_CENTER:-0.43}"
CALIB_MID_BANDWIDTH="${CALIB_MID_BANDWIDTH:-0.14}"
CALIB_LOW_CEIL="${CALIB_LOW_CEIL:-0.09}"
LOSS_CALIBRATION_HIGH_WEIGHT="${LOSS_CALIBRATION_HIGH_WEIGHT:-2.2}"
LOSS_CALIBRATION_MID_WEIGHT="${LOSS_CALIBRATION_MID_WEIGHT:-1.4}"
LOSS_CALIBRATION_LOW_WEIGHT="${LOSS_CALIBRATION_LOW_WEIGHT:-1.8}"
...
STAGE2_OOB_HIGH_WEIGHT="${STAGE2_OOB_HIGH_WEIGHT:-3.2}"
STAGE2_OOB_MID_WEIGHT="${STAGE2_OOB_MID_WEIGHT:-2.6}"
STAGE2_OOB_LOW_WEIGHT="${STAGE2_OOB_LOW_WEIGHT:-3.0}"
...
STAGE2_GATED_RECALL_MIN="${STAGE2_GATED_RECALL_MIN:-0.72}"
...
log "Starting ce/train2.py (domain+method+optional constraint)"
log "raw_input=${RAW_INPUT}"
log "method_raw_input=${METHOD_RAW_INPUT}"
log "constraint_raw_input=${CONSTRAINT_RAW_INPUT}"
