...
from ce.llm_runtime_util import (
    batched as _util_batched,
    build_prompt as _util_build_prompt,
    clean_text as _util_clean_text,
    coerce_score as _util_coerce_score,
    extract_json_object as _util_extract_json_object,
)

# Local augmentation helper.
try:
    from ce.data_preparation.llm_distillation.augmentation import LLMDistillationAugmenter
except Exception:
    try:
...
PREFILTER_CACHE_DEFAULT = "ce/dataset/source/spec_facspec_sts_cache.jsonl"
RAW_OUTPUT_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_raw_scores.jsonl"
PAIRWISE_OUTPUT_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_pairwise.jsonl"
LISTWISE_OUTPUT_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_listwise.jsonl"
MANIFEST_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_manifest.json"
STATS_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_stats.json"
DISTILL_RAW_TEMPLATE = "ce/dataset/distill/llm_distill_{aspect}_distill_raw_scores.jsonl"
...
AUGMENT_MAX_ATTEMPTS_DEFAULT = 5
...
CONSTRAINT_USER_PROMPT_TEMPLATE = """
Grant specialization keyword:
{spec_text}
...
PROMPT_CONFIGS: Dict[str, Tuple[str, str]] = {
    "domain": (DOMAIN_SYSTEM_PROMPT, DOMAIN_USER_PROMPT_TEMPLATE),
    "method": (METHOD_SYSTEM_PROMPT, METHOD_USER_PROMPT_TEMPLATE),
    "constraint": (CONSTRAINT_SYSTEM_PROMPT, CONSTRAINT_USER_PROMPT_TEMPLATE),
}


def _clean_text(value: Any) -> str:
    return _util_clean_text(value)
...
def _clamp_score(value: Any) -> float:
    return _util_coerce_score(value)
...
def _extract_score(raw_text: str) -> Tuple[float, bool]:
    ...
    obj = _util_extract_json_object(raw)
...
def _chunked(items: Sequence[Any], batch_size: int) -> List[Sequence[Any]]:
    ...
    return list(_util_batched(items, size))
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional override for Stage-A raw scored spec file path. "
            "If empty, uses aspect-specific default template."
        ),
    )
    parser.add_argument(
        "--judge-aspect",
        type=str,
        default="domain",
        choices=["domain", "method", "constraint"],
        help="Judge aspect/prompt set to run.",
    )
    ...
    raw_output_path = _resolve_path(RAW_OUTPUT_TEMPLATE.format(aspect=judge_aspect))
    pairwise_output_path = _resolve_path(PAIRWISE_OUTPUT_TEMPLATE.format(aspect=judge_aspect))
    listwise_output_path = _resolve_path(LISTWISE_OUTPUT_TEMPLATE.format(aspect=judge_aspect))
    manifest_path = _resolve_path(MANIFEST_TEMPLATE.format(aspect=judge_aspect))
    stats_output_path = _resolve_path(STATS_TEMPLATE.format(aspect=judge_aspect))
    if _clean_text(args.distill_raw_path):
        distill_raw_path = _resolve_path(args.distill_raw_path)
    else:
        distill_raw_path = _resolve_path(DISTILL_RAW_TEMPLATE.format(aspect=judge_aspect))
