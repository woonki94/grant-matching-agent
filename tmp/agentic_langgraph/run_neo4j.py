...
from tmp.agentic_langgraph.workflow import ProfessionGrantLangGraphOrchestrator
from tmp.neo4j_common import json_ready, load_dotenv_if_present, read_neo4j_settings


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run LangGraph faculty<->grant conversational orchestration on Neo4j.")
...
def main() -> int:
    ...
    try:
        orchestrator = ProfessionGrantLangGraphOrchestrator(
        ...
        payload = {
            ...
            "graph_mermaid": orchestrator.draw_mermaid(),
        }
